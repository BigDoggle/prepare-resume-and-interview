...
这个 Skill 会先整理经历，再开始写简历。旧简历记录背景，代码反映实现，Git 保留开发过程，测试说明功能边界。结合这些材料，可以还原完整的项目过程，分清团队工作和个人贡献，再筛选适合写进简历的内容。

写出来的内容也要经得起追问。以并发问题为例，Skill 会记录实际完成的状态控制、事务校验和测试闭环，并按证据确定职责范围。简历要点、项目介绍和面试问题共用同一份事实底稿，面试时可以继续讲清设计选择、异常路径和验证方式。
